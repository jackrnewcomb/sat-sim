...
struct Duration {
  ...
  static constexpr Duration from_minutes(double m) {
    return Duration{60.0 * m};
  }
  static constexpr Duration from_hours(double h) {
    return Duration{3600.0 * h};
  }
  static constexpr Duration from_days(double d) {
    return Duration{86400.0 * d};
  }
};
...
class Time {
 public:
  ...
  static constexpr Time from_ns_since_j2000(std::int64_t ns) {
    return Time{ns};
  }
  ...
  double seconds_since_j2000() const {
    return static_cast<double>(ns_since_j2000_) * 1e-9;
  }

  double julian_date() const {
    return JD_J2000 + (seconds_since_j2000() / 86400.0);
  }
  ...
  // Arithmetic
  friend Time operator+(Time t, Duration d) {
    return Time{t.ns_since_j2000_ + to_ns(d.seconds)};
  }
  friend Time operator-(Time t, Duration d) {
    return Time{t.ns_since_j2000_ - to_ns(d.seconds)};
  }
  friend Duration operator-(Time a, Time b) {
    return Duration{
        (static_cast<double>(a.ns_since_j2000_ - b.ns_since_j2000_) * 1e-9)};
  }
 ...
 private:
  ...
  static std::int64_t to_ns(double seconds) {
    ...
    if (ns > static_cast<double>(INT64_MAX) ||
        ns < static_cast<double>(INT64_MIN)) {
      throw std::overflow_error(
          "Time overflow converting seconds to nanoseconds");
    }
  ...
  static double utc_to_jd(const UtcCalendar& utc) {
    ...
    const double JD = std::floor(365.25 * (Y + 4716)) +
                      std::floor(30.6001 * (M + 1)) + static_cast<double>(D) +
                      day_fraction + static_cast<double>(B) - 1524.5;
